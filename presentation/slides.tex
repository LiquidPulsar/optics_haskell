...
\begin{frame}[plain]
  \centering
  \vfill
  {\Huge\bfseries\color{imperialblue} Live demonstration}\\[1.2em]
...
\begin{frame}[fragile]{Demo, part 1: the compiler does the kernel-size arithmetic (live)}
...
\end{lstlisting}
  ...
  whole chain, the flattened width is no longer \texttt{125}, and the dense layer stops compiling.
...
\end{lstlisting}
  \centering\footnotesize I wrote none of those intermediate sizes. A wrong kernel is a type error,
  not a runtime crash.
\end{frame}
